...
public class MMRDS implements RoutingAlgorithmInterface {
    ...
    private List<Route> computeRoutes(Node n1, Node n2, Mesh mesh) {
        ...
        Double sizeRoute = Double.POSITIVE_INFINITY;
        Route r = new Route();
        r.addNode(n1);
        ...
        while (!routesUnderConstruction.isEmpty()) {
            ...
            if (expand.getDestination().equals(n2)) { //Route completed
                ...
                if (expand.getHops() < sizeRoute) { //Check if you found a route smaller than the others already found
                    sizeRoute = (double)expand.getHops();
            ...
            if (expand.getHops() >= sizeRoute) { //It's no use keeping looking on this route
                continue;
            }

            //Search more routes from this
            for (Node no : mesh.getAdjacentsNodes(expand.getDestination())) {
