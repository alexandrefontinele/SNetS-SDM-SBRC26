...
import gprmcsa.trafficGrooming.util.AuxiliaryGraph;
import gprmcsa.trafficGrooming.util.MicroRegion;
...
public class AuxiliaryGraphGrooming implements TrafficGroomingAlgorithmInterface {
    ...
    private boolean expandCircuits(List<AuxiliaryGraph.Edge> solution, RequestForConnection rfc, ControlPlane cp){
        ...
        for(AuxiliaryGraph.Edge e: solution){
            ...
            int slotsNeeded = c.getModulation().requiredSlots(c.getRequiredBitRate() + rfc.getRequiredBitRate()) - (c.getSpectrumAssigned()[1] - c.getSpectrumAssigned()[0] + 1);
            if(slotsNeeded>0){
    ...
    public class MyEdge implements AuxiliaryGraph.Edge{
        ...
        private void analise(){
            ...
            this.spectrumUtilization = circuit.getModulation().requiredSlots(rfc.getRequiredBitRate()) * circuit.getRoute().getHops();
