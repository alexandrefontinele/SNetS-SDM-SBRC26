...
/**
 * Represents the MultihopGrooming component.
 */
public abstract class MultihopGrooming implements TrafficGroomingAlgorithmInterface {
	...
	/**
	 * Executes the init virtual routing operation.
	 * @param cp the cp.
	 */
	...
	/**
	 * Executes the init micro regions operation.
	 * @param cp the cp.
	 */
	...
	/**
	 * Returns the make loop.
	 * @param sol the sol.
	 * @param newCirc the newCirc.
	 * @return true if the condition is met; false otherwise.
	 */
	private static boolean makeLoop(ArrayList<Circuit> sol, Circuit newCirc) {

		for (Circuit c : sol) {
			if (c.getSource().getName().equals(newCirc.getSource().getName()))
				return true;
			if (c.getSource().getName().equals(newCirc.getDestination().getName()))
				return true;
		}
		return false;
	}
	...
	private static boolean hasVirtualLoop(MultihopSolution sol) {
		...
		if (sol.needsComplement && nAux.contains(sol.pairComplement.getDestination().getName()))
			return true;
	...
	/**
	 * Adds the new circuit virtual routing.
		...
		for (ArrayList<MultihopSolution> multihopSolutions : this.virtualRouting.get(dst).values()) {
			for (MultihopSolution ms : multihopSolutions) {
				if (ms.virtualRoute.size() == 0
						&& !microRegions.get(ms.src).contains(ms.pairComplement.getDestination()))
					continue;
				if (ms.virtualRoute.size() == this.maxVirtualHops)
					continue;
				if (makeLoop(ms.virtualRoute, circuit))
					continue;
	 ...
	 */
	/*
	 * private void updateSNRImpact(Circuit c){ Route r = c.getRoute(); for(Link l:
	 * r.getLinkList()){ for(Circuit ci: l.getCircuitList()){
	 * oSNRImpactPerCircuit.remove(ci); } } } /** this method must be called when a
	 * circuit is added or removed.
	 *
	 * @param c
	 * Created by Selles
	 * Added a for-loop to iterate over cores because the circuit list exists only inside the core
	 */
	private void updateSNRImpact(Circuit c) {
		Route r = c.getRoute();
		for (Link l : r.getLinkList()) {
			for (Core core : l.getCores()) {
				for (Circuit ci : core.getCircuitList()) {
	...
	/**
	 * Removes the circuit virtual routing.
	 * @param circuit the circuit.
	 */
	...
	/**
	 * Returns the all virtual routing.
	 * @param rfc the rfc.
	 * @return the result of the operation.
	 */
	private ArrayList<MultihopSolution> allVirtualRouting(RequestForConnection rfc) {
		...
		allSolutions = (ArrayList<MultihopSolution>) this.virtualRouting.get(rfc.getPair().getSource().getName())
				.get(rfc.getPair().getDestination().getName());
	 ...
	 * @return the result of the operation.
	 */
	protected Circuit complementSolution(MultihopSolution ms, RequestForConnection rfc, ControlPlane cp)
			throws Exception {
	...
	/**
	 * Checks whether suficient residual capacity.
	 * @param ms the ms.
	 * @param rfc the rfc.
	 * @return true if the condition is met; false otherwise.
	 */
	private boolean hasSuficientResidualCapacity(MultihopSolution ms, RequestForConnection rfc) {
		...
		Circuit circuit;
		for (Iterator<Circuit> var4 = ms.virtualRoute.iterator(); var4
				.hasNext(); hasSuficientResitualCapacity = hasSuficientResitualCapacity
						&& circuit.getResidualCapacity() > rfc.getRequiredBitRate()) {
			circuit = var4.next();
		}
	...
	private boolean canBeExpanded(Circuit c, RequestForConnection rfc) {
		...
		int slotsNeeded = c.getModulation().requiredSlots(c.getRequiredBitRate() + rfc.getRequiredBitRate())
				- (c.getSpectrumAssigned()[1] - c.getSpectrumAssigned()[0] + 1);
		return circExCap >= slotsNeeded;
	}

	/**
	 * Returns the aply solution.
	 * @param ms the ms.
	 * @param rfc the rfc.
	 * @param cp the cp.
	 * @return true if the condition is met; false otherwise.
		...
		if (hasSuficientResitualCapacity) {
			...
			for (i = 0; i < ms.virtualRoute.size(); ++i) {
				...
				int slotsNeeded = c.getModulation().requiredSlots(c.getRequiredBitRate() + rfc.getRequiredBitRate())
						- (c.getSpectrumAssigned()[1] - c.getSpectrumAssigned()[0] + 1);
				if (slotsNeeded > 0) {
					List<int[]> composition = IntersectionFreeSpectrum.merge(c.getRoute(), c.getGuardBand(), c.getIndexCore());
					int bandFreeAdjInferior = IntersectionFreeSpectrum.freeSlotsDown(c.getSpectrumAssigned(),
							composition, c.getGuardBand());
					int bandFreeAdjSuperior = IntersectionFreeSpectrum.freeSlotsUpper(c.getSpectrumAssigned(),
							composition, c.getGuardBand());
	...
	public void finishConnection(RequestForConnection rfc, ControlPlane cp) throws Exception {
		...
		for (Circuit circuit : rfc.getCircuits()) {
			if (circuit.getRequests().size() == 1) {
				...
				int numFinalSlots = circuit.getModulation()
						.requiredSlots(circuit.getRequiredBitRate() - rfc.getRequiredBitRate());
	...
	private void avMS(ArrayList<MultihopSolution> ams, RequestForConnection rfc, ControlPlane cp) {
		...
		// normalization
		if (snrImpact == 0)
			snrImpact = 1; // avoid NaN
		if (meanSNR == 0)
			meanSNR = 1; // avoid NaN
		if (transceivers == 0)
			transceivers = 1; // avoid NaN
		if (minSNR == 0)
			minSNR = 1; // avoid NaN
		if (spectrumUtilization == 0)
			spectrumUtilization = 1; // avoid NaN
		if (physicalHops == 0)
			physicalHops = 1; // avoid NaN
		if (virtualHops == 0)
			virtualHops = 1; // avoid NaN
	...
	private void printVirtualRoute() {
		...
		for (String o : virtualRouting.keySet()) {
			for (String d : virtualRouting.get(o).keySet()) {
				if (virtualRouting.get(o).get(d).size() < 2)
					continue;
	...
	protected class MultihopSolutionStatistics {
		...
		MultihopSolutionStatistics(MultihopSolution sol, RequestForConnection rfc, ControlPlane cp,
				MSSOptimizator msso) {
			...
			for (Circuit circuit : sol.virtualRoute) {
				...
				circuit.getModulation().requiredSlots(rfc.getRequiredBitRate());
				this.spectrumUtilization += (double) (circuit.getModulation().requiredSlots(rfc.getRequiredBitRate())
						* circuit.getRoute().getHops());
				snr = circuit.getSNR();
				if (snr < this.minSNR) {
					this.minSNR = snr;
				}
				...
				if (msso.oCanEstabilish.get(sol.pairComplement.toString()) == null) {
					...
					try {
						if (!cp.establishCircuit(newCircuit)) {
							...
							newCircuit.removeRequest(rfc);
							msso.oPhysicalHops.put(sol.pairComplement.toString(),
									(double) newCircuit.getRoute().getHops());
							msso.oCanEstabilish.put(sol.pairComplement.toString(), true);
							msso.oSpectrumUtilization.put(sol.pairComplement.toString(),
									(double) (newCircuit.getModulation().requiredSlots(rfc.getRequiredBitRate())
											* newCircuit.getRoute().getHops()));
