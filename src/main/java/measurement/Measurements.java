...
public class Measurements implements Serializable {
    ...
    private void initializeMetrics(Mesh mesh){
		...
		if(measuringMetrics.BitRateBlockingProbability){
			BitRateBlockingProbability probabilidadeDeBloqueioDeBanda = new BitRateBlockingProbability(loadPoint, replication, mesh.getUtil());
		...
		if(measuringMetrics.DataSetInformation){
			DataSetInformation dataSetInformation = new DataSetInformation(loadPoint, replication, mesh.getUtil());
			this.metricsList.add(dataSetInformation);
		}
		if(measuringMetrics.PhysicalLayerStatistics){
			PhyscialLayerStatistics physicalLayerStatistics = new PhyscialLayerStatistics(loadPoint, replication, mesh.getUtil());
