...
//	}
	...
	public boolean isAdmissibleInOthersImprimirLog(Circuit circuit) {
		...
		for (Circuit circuitNeighbor : circuitosAdj) {
			...
			System.out.println("--O crosstalk no caminho óptico "+circuitNeighbor.getId()+", já ativo no núcleo "+circuitNeighbor.getIndexCore()+", após a alocação do caminho óptico "+circuit.getId()+" será de: "+newXt+" (limiar: "+circuitNeighbor.getModulation().getXTthreshold()+")");
	...
	private void printTest(Circuit circuit, String origem, String destino, double totalXT) {
		if (circuit.getSource().getName().equals(origem) && circuit.getDestination().getName().equals(destino)) {
			...
			System.out.println("Crosstalk para o circuito tendo o core: "+circuit.getIndexCore()+" e slots: "+circuit.getSpectrumAssigned()[0]+"-"+circuit.getSpectrumAssigned()[1]+" eh "+PhysicalLayer.ratioForDB(totalXT)+" Com mudulação: "+circuit.getModulation().getName()+" portanto: "+isAdmissible(circuit, PhysicalLayer.ratioForDB(totalXT)));
