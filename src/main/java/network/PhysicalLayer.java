...
public class PhysicalLayer implements Serializable {
	...
	public double computeSNRSegment(Circuit circuit, Route route, int sourceNodeIndex, int destinationNodeIndex, Modulation modulation, int core, int spectrumAssigned[], Circuit testCircuit, boolean addTestCircuit){
		...
		System.out.println("---------------------\nHabibi:\nDistância da Rota: "+circuit.getRoute().getDistanceAllLinks()+"\nNúmero de spans: "+circuit.getRoute().getDistanceAllLinks()/100+"\nBsi: "+Bsi+"\nModulação: "+circuit.getModulation().getName()+"\nQuantidade de slots: "+(circuit.getSpectrumAssigned()[1]-circuit.getSpectrumAssigned()[0])+"\nPxt: "+ratioForDB(juraPxt));
	...
	public static double arcsinh(double x){
	    ...
	    // 1. Expansão de Taylor para x muito pequeno
	    if (absX < 1e-4) {
	        double x2 = x * x;
	        return x * (1.0 - x2 * (1.0 / 6.0 - x2 * (3.0 / 40.0)));
	    }

	    // 2. Aproximação assintótica para x muito grande
	    if (absX > 1e6) {
	    ...
	    // 3. Região intermediária — cálculo padrão com estabilidade numérica
	    // Usa log1p para maior precisão em valores médios/pequenos
...
//		}
	...
	// Novo Método para computar distância da modulação
	...
	//Novo método para computar distâncias
	public double computeModulationDistanceByBandwidth3(Modulation mod, double bitRate, Mesh mesh) {
		...
		double minDistance = 12.5; //km, distancia minima
		double totalDistance = 50000.0; //km, distancia maxima
		int quantSpansPorEnlace = (int)(totalDistance / minDistance); // number of spans per link
		...
		for(int ns = 1; ns <= quantSpansPorEnlace; ns++){
			...
			int Nch = 15; //quantidade de circuitos
			...
			for (int c = 1; c <= Nch; c++) {
				...
				saTemp = sa[1]; //guarda o slot final do circuito
	...
	public double computeModulationDistanceByBandwidth4(Modulation mod, double bitRate, Mesh mesh) {
		...
		double minDistance = 100.0; //km, distancia minima
		double totalDistance = 50000.0; //km, distancia maxima
		int quantSpansPorEnlace = (int)(totalDistance / minDistance); // number of spans per link
...
/*	public double computeModulationDistanceByBandwidth(Modulation mod, double bitRate, Mesh mesh) {
		...
		for(int ns = 0; ns <= quantSpansPorEnlace; ns++){
			...
			route.getLink(0).getCore(0).addCircuit(circuitTemp); // Verificar!!!
...
*/	
	...
	public double computePowerThatMaximizesOSNR(Circuit circuit, double bitRate, Route route, int sourceNodeIndex, int destinationNodeIndex, Modulation modulation, int core, int spectrumAssigned[]){
		...
		// Protege contra divisão por zero preservando sinal
	...
	private double checkMaximumPower(double Pstart, Circuit circuit, Route route, Modulation mod, int core, int sa[]) {
		...
		// =============================================
		// Expansão adaptativa do intervalo [Pmin, Pmax] a partir de P0
		// Detectando automaticamente a região do máximo.
		// =============================================
	    
	    // Garante que Pstart está dentro dos limites físicos
	    ...
	    if (OSNR_left < OSNR_central && OSNR_right < OSNR_central) { // Caso P0 já seja um máximo local
	        Pmin = P_left;
	        Pmax = P_right;
	    }
	    else if (OSNR_right > OSNR_central) { // Expansão para a direita
	        Pmin = P0;
	        Pmax = P_right;
	    ...
	    else if (OSNR_left > OSNR_central) { // Expansão para a esquerda
	        Pmax = P0;
	        Pmin = P_left;
		...
		// =============================================
		// Busca ternária para máximo de OSNR no intervalo [Pmin, Pmax]
		// =============================================
	    final int    MAX_TERNARY_ITER   = 50;
	    final double MIN_POWER_TER = 1e-15;
	    
	    // Melhor tolerância híbrida para potenciais ranges amplos
	    final double ABS_TOL = 1e-12;   // seguro para potências minúsculas (1e-12 W)
	    final double REL_TOL = 1e-3;    // 0.1% de tolerância relativa
	    ...
	    for (int i = 0; i < MAX_TERNARY_ITER; i++) {
			...
			if (OSNRp1 < OSNRp2) {
				Pmin = p1; // máximo está à direita
			} else {
				Pmax = p2; // máximo está à esquerda
			}
	    ...
	    // Retorna o ponto médio refinado
	...
	public void testCamadaFisica() {
		...
		//Adicionando os circuitos na rede
		...
		//Calculando o OSNR de cada circuito
	...
	public void testCamadaFisicaXT() {
	    ...
	    // Configuração de núcleos (MCF 7-core)
	    List<CoreConfig> coreList = new ArrayList<>();
	    coreList.add(new CoreConfig(0, new ArrayList<>(Arrays.asList(1, 2, 3, 4, 5, 6))));
	    coreList.add(new CoreConfig(1, new ArrayList<>(Arrays.asList(0, 2, 6))));
	    coreList.add(new CoreConfig(2, new ArrayList<>(Arrays.asList(0, 1, 3))));
	    ...
	    // ============================
	    // Circuito 1 (referência) no core 0
	    // ============================
	    double tr1 = 100.0E+9; // bps
	    ...
	    // ============================
	    // Circuitos no MESMO núcleo (NLI / interferência intra-core)
	    // ============================
	    double tr2 = 100.0E+9;
	    int slotNumber2 = 3;
	    int sa2[] = new int[2];
	    ...
	    // ============================
	    // Circuitos em núcleos adjacentes (geram XT no circuito 1)
	    // ============================
	    // Todos com a MESMA faixa de slots de c1 ([1,3]) para maximizar XT
	    int[] saXT = new int[]{sa1[0], sa1[1]};
	    double trXT = 100.0E+9;
	    
	    List<Circuit> xtNeighbors = new ArrayList<>();
	    int[] adjacentCores = new int[]{1,2,3,4,5,6}; // núcleos vizinhos do core 0
	    ...
	    // ============================
	    // Adicionando todos os circuitos na rede
	    // ============================
	    ...
	    // ============================
	    // Varredura de potência para gerar curvas OSNR x P e XT x P
	    // ============================
	    
	    System.out.println("#P_dBm; OSNR_dB; XT_dB");
	    
	    // exemplo: varre de -5 dBm até +5 dBm, passo de 0.5 dB
	    for (double P_dBm = -5.0; P_dBm <= 5.0; P_dBm += 0.5) {
	        
	        // converte dBm para Watts
	        double P_Watts = dBm_to_W(P_dBm); // use seu método real
	        
	        // configura a potência de lançamento do circuito 1
	        ...
	        // XT que o circuito 1 sofre dos vizinhos (inter-core + intra-core)
	        // Aqui você deve usar a função real que já existe no seu PhysicalLayer/Crosstalk
