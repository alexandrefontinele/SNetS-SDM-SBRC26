...
public class SimulationManagement {
    ...
    public static List<List<Simulation>> createAllSimulations(SimulationRequest sr){
        ...
        List<List<Simulation>> allSimulations = new ArrayList<>(); // Each element of this set is a list with all replications from the same load point
        for (int i = 0; i < sc.getLoadPoints(); i++) { // Create the simulations for each load point
            List<Simulation> reps = new ArrayList<>();
            for (int j = 0; j < sc.getReplications(); j++) { // Create the simulations for each replication
