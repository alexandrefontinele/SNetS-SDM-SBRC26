...
public class ExternalFragmentationResultManager implements ResultManagerInterface {
	...
	public String result(List<List<Measurement>> llms){
		...
		res.append("\n\n");
		res.append(resultGeneralEntropyVertical());
		res.append("\n\n");
		res.append(resultGeneralHorizontal());
		res.append("\n\n");
		res.append(resultPerLink());
		res.append("\n\n");
		res.append(resultEntropyPerLink());
		res.append("\n\n");
	...
	/**
	 * Returns the entropy external fragmentation (vertical)
	 *
	 * @return String
	 */
	private String resultGeneralEntropyVertical() {
		StringBuilder res = new StringBuilder();
		for (Integer loadPoint : loadPoints) {
			res.append("Entropy External Fragmentation (Vertical)" + sep + loadPoint + sep + "all" + sep + " ");
			for (Integer replic : replications) {
				res.append(sep + efs.get(loadPoint).get(replic).getEntropyExternalFragVertical());
			}
			res.append("\n");
		}
		return res.toString();
	...
	/**
	 * Returns the entropy external fragmentation per link
	 *
	 * @return String
	 */
	private String resultEntropyPerLink(){
		StringBuilder res = new StringBuilder();
		for (Integer loadPoint : loadPoints) {
			String aux = "Entropy External Fragmentation Per Link" + sep + loadPoint + sep; // "all"+sep+" ";

			for (String link : efs.get(0).get(0).getLinkSet()) {
				String aux2 = aux + "<" + link + ">" + sep + " ";

				for (Integer replic : replications) {
					aux2 = aux2 + sep + efs.get(loadPoint).get(replic).getEntropyExternalFragLink(link);
				}
				res.append(aux2 + "\n");
			}
